...
class BiLSTM(nn.Module):
    ...
    def __init__( self, input_dim=22, lstm_hidden_dim=256, hidden_3_dim=1024, rri_size=2 ):
        super(BiLSTM, self).__init__()

        self.input_dim = input_dim
        self.lstm_hidden_dim = lstm_hidden_dim
        self.hidden_3_dim = hidden_3_dim
        self.conv2d_channels_added = 8
        self.n_conv2d = 2
        ...
        self.hidden2hidden_3 = nn.Linear(4*self.lstm_hidden_dim, hidden_3_dim)
    ...
    def forward( self, pdb, sequence_r, sequence_l, features, contacts, ch_r=None, ch_l=None, Flag=True ):
        ...
        dim_2 = 2*self.lstm_hidden_dim
        dim_4 = 4*self.lstm_hidden_dim

        _out_LSTM_r = torch.cat((out_LSTM_r.squeeze(1), autograd.Variable( torch.zeros(N_r,dim_2)).cuda() ),dim=1)
        _out_LSTM_l = torch.cat((autograd.Variable( torch.zeros(N_l,dim_2)).cuda(), out_LSTM_l.squeeze(1)),dim=1)

        pre_conv = _out_LSTM_r.repeat(N_l,1,1).permute(1,0,2) + _out_LSTM_l.repeat(N_r,1,1)

        _out_LSTM_r = torch.cat((autograd.Variable( torch.zeros(N_r,dim_2)).cuda(),out_LSTM_r.squeeze(1) ),dim=1)
        _out_LSTM_l = torch.cat((out_LSTM_l.squeeze(1),autograd.Variable( torch.zeros(N_l,dim_2)).cuda()),dim=1)
...
model = BiLSTM( input_dim=input_dim, lstm_hidden_dim=lstm_hidden_dim, hidden_3_dim=hidden_3_dim, rri_size=2 )
model.cuda()
