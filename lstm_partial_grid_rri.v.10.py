...
training['ignored'] = read_ignore("ignore_dimers_450_list.tsv","dimers_450_ignore")
...
testing['ignored'] = read_ignore("ignore_dimers_bipspi_list.tsv","dimers_bipspi_ignore")
...
class BiLSTM(nn.Module):
    ...
    def forward( self, pdb, sequence_r, sequence_l, features, contacts, ignore, ch_r=None, ch_l=None, Flag=True ):
        ...
        if Flag:
          ...
          while N_rri>0:
            ...
            #if verbose:
            #  if sequence_r[i_r][:-1] in ignore[pdb][ch_r]:
            #    print("\tresidue %s %s:%s excluded"%(pdb,sequence_r[i_r][:-1],ch_r))
            #  if sequence_l[j_l][:-1] in ignore[pdb][ch_l]:
            #    print("\tresidue %s %s:%s excluded"%(pdb,sequence_l[j_l][:-1],ch_l))

            if (not sequence_r[i_r]+":"+sequence_l[j_l] in contacts['rri'][pdb]) and (not sequence_r[i_r][:-1] in ignore[pdb][ch_r]) and (not sequence_l[j_l][:-1] in ignore[pdb][ch_l]):
              w_rc = out_hidden_r_2[i_r,:]
              w_lc = out_hidden_l_2[j_l,:]
          ...
          v_in = autograd.Variable( torch.FloatTensor(N_r_*N_l_,2*self.hidden_2_dim) ).cuda()
          v_in_t = autograd.Variable( torch.FloatTensor(N_r_*N_l_,2*self.hidden_2_dim) ).cuda()
          ...
          for i_r in range( N_r ):
            for j_l in range( N_l ): 
              ...
              #  if sequence_l[j_l][:-1] in ignore[pdb][ch_l]:
              #    print("\t>residue %s %s:%s excluded"%(pdb,sequence_l[j_l][:-1],ch_l))
              if (sequence_r[i_r][:-1] in ignore[pdb][ch_r]) or (sequence_l[j_l][:-1] in ignore[pdb][ch_l]):
                continue
...
for target_ in TRAGETS:
  ...
  if os.path.isfile(results_path+"/"+target+".tsv"):
    ...
    open(results_path+"/"+target+".tsv",'a')
  ...
  for epoch in range(n_epoch):
    ...
    for pdb in cci_:
      for ch_ch in training['contacts']['cci'][pdb]:
        ...
        predicted_rri, native_rri, out_hidden_3 = model( pdb, local_sequence_r, local_sequence_l, training['features'], training['contacts'], training['ignored'], ch_r=ch_r, ch_l=ch_l, Flag=True )
    ...
    for ch_ch in testing['contacts']['cci'][target]:
      ...
      predicted_rri, native_rri, out_hidden_3 = model( target, local_sequence_r, local_sequence_l, testing['features'], testing['contacts'], testing['ignored'], ch_r=ch_r, ch_l=ch_l, Flag=False )
