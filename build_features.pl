...
my $pwd = "/home/jsegura/databases/DBD4/B3/";

my @in = `ls $pwd | grep _u.pdb | grep -v "pdb.2"`;
chomp @in;
...
foreach my$i(@in){
	...
	#my ($f_rsa,$sequence) = read_naccess("NACCESS/$sufix.rsa");
	my ($f_psaia,$sequence,$ch_sequence) = read_psaia("PSAIA/results/".$sufix."_201602261134_bound.tbl");

	my ($__nn,$coo) = compute_neighbours->nearest("$pwd/$i",15);

	write_nn_features($sufix,$f_psaia,$sequence,$ch_sequence,$__nn,$coo);
}
