...
class BiLSTM(nn.Module):
    ...
    def forward(self, pdb_r, pdb_l, sequence_r, sequence_l ):

        v_r, embed_r = self.prepare_data( pdb_r, sequence_r )
        v_l, embed_l = self.prepare_data( pdb_l, sequence_l )
...
def get_native_rri( pdb_r, pdb_l, ch_r, ch_l ):

  BS = []
  for aa in all_sequence[pdb_r][ch_r]:
    if aa in pdb_bs[pdb_r]:
      BS.append(1)
    else:
      BS.append(0)
  BS.append(0)
  for aa in all_sequence[pdb_l][ch_l]:
    if aa in pdb_bs[pdb_l]:
      BS.append(1)
    else:
      BS.append(0)
  ...
  for epoch in range(200):
    ...
    for pdb in cci:
      ...
      for ch_ch in cci[pdb]:
        ...
        local_sequence_r = all_sequence[pdb_r][ch_r]
        local_sequence_l = all_sequence[pdb_l][ch_l]

        model.zero_grad()
        optimizer.zero_grad()

        predicted_rri = model( pdb_r,  pdb_l, local_sequence_r, local_sequence_l )
        native_rri =  get_native_rri( pdb_r, pdb_l, ch_r, ch_l )
    ...
    for ch_ch in cci[target]:
      ...
      np_class_1 = native_rri.data.cpu().numpy() 
      np_prediction_1 = predicted_rri.data.cpu()[:,1].numpy()
      ...
      np_class_2 = native_rri.data.cpu().numpy() 
      np_prediction_2 = predicted_rri.data.cpu()[:,1].numpy()


      fpr, tpr, thresholds = metrics.roc_curve(np_class_1[0:N_r], 0.5*(np_prediction_1[0:N_r]+np_prediction_2[N_l+1:]), pos_label=1)
      auc_r = metrics.auc(fpr, tpr)

      fpr, tpr, thresholds = metrics.roc_curve(np_class_1[N_r+1:], 0.5*(np_prediction_1[N_r+1:]+np_prediction_2[0:N_l]), pos_label=1)
